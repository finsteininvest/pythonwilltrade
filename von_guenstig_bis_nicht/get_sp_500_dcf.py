...
for index, row in symbole_df.iterrows():
    ticker = row['ticker']
    name = row['name']
    dcf_json = requests.get(f"https://financialmodelingprep.com/api/v3/company/discounted-cash-flow/{ticker}?apikey={api_key}")
    ...
    try:
        ...
        delta_prc = float(f'{delta_prc:.2f}')
        dcf = float(f"{dcf_json['dcf']:.2f}")
        print(f"{name}, DCF: {dcf}, latest close: {dcf_json['Stock Price']}, Delta: {delta}, {delta_prc}%")
        dcf_delta_list.append([name, ticker, dcf, dcf_json['Stock Price'], delta, delta_prc])
        sleep(1)
    except:
        pass

dcf_delta_df = pd.DataFrame.from_records(dcf_delta_list, columns=['name','ticker','dcf', 'latest close', 'delta', '%'])
